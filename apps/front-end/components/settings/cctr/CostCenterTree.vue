...
const onDelete = (data: CostItem) => {
	if (props.employees === 0) {
		useCFetch<Response<any>>(`/api/v2/settings/costCenters/${data.id}`, {
			method: "DELETE",
			params: { id: data.id },
			body: data,
...
const onSubmit = (data: CostParams) => {
	validate()
		.then(async () => {
			...
			const url =
				data.method === "PATCH"
					? `/api/v2/settings/costCenters/${data.id}`
					: "/api/v2/settings/costCenters"
...
<template>
	<a-flex gap="small" justify="flex-end" align="center" class="mb-md" wrap="wrap">
		<eacc-excel-button
			ghost
			type="primary"
			url="/api/v2/settings/costCenters/validate"
			req-type="upload"
			label="엑셀일괄등록"
		...
		<template #content="{ field }">
			...
			>
				...
				<a-form-item label="상위 코스트센터">
					<eacc-select
						:url="`/api/v2/settings/commons/costCenters?companyCode=${field.companyCode}&parentId=${field.companyId}`"
