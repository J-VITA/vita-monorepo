...
const { data, status, refresh } = await useAsyncData(
	`cctr-employees-list`,
	() =>
		useCFetch<Response<any>>("/api/v2/settings/costCenters/employees", {
			method: "GET",
			params: {
...
const onDelete = async (data: number[]) => {
	...
	await useCFetch<Response<any>>(
		`/api/v2/settings/costCenters/employees/${props.info?.id}`,
		{
			method: "DELETE",
			body: {
				id: props.info?.id,
	...
	<a-flex gap="small" justify="space-between" align="center" class="mb-md" wrap="wrap">
		...
		<a-space :size="5">
			...
			<eacc-excel-button
				ghost
				type="primary"
				url="/api/v2/settings/costCenters/employees/validate"
				req-type="upload"
				label="엑셀일괄등록"
