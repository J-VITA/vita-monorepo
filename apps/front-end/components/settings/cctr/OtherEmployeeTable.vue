...
const {
	...
	() =>
		useCFetch<Response<any>>("/api/v2/settings/costCenters/roles", {
			method: "GET",
			params: {
...
const onDelete = (data: number[]) => {
	...
	data.forEach(async (id) => {
		await useCFetch<Response<any>>(`/api/v2/settings/costCenters/roles/${id}`, {
			method: "DELETE",
			body: {
				id: id,
			},
...
const onAddSubmit = async (data: AddRef) => {
	await useCFetch<Response<any>>("/api/v2/settings/costCenters/roles", {
		method: "POST",
		body: {
			id: info?.id,
			companyCode: info?.companyCode,
...
const authChangeSubmit = async (params: AuthChangeParams) => {
	await useCFetch<Response<any>>(`/api/v2/settings/costCenters/roles/${params.type}`, {
		method: "PATCH",
		body: {
			id: info?.id,
			companyCode: info?.companyCode,
