...
const showPositionModal = ref<boolean>(false)
...
const columns = ref<ColumnsType<Positions>>([
	{
		...
		dataIndex: "gradeName",
		resizable: true,
		sorter: (a: Positions, b: Positions) => a.gradeName!.length - b.gradeName!.length,
		width: -1,
	},
	{
		title: "이름",
	...
	},
	{
		...
		resizable: true,
		sorter: (a: Positions, b: Positions) =>
	...
	},
	{
		...
		dataIndex: "jobName",
		resizable: true,
		sorter: (a: Positions, b: Positions) => a.jobName!.length - b.jobName!.length,
		width: -1,
	},
	{
		title: "이메일",
		dataIndex: "email",
		resizable: true,
		sorter: (a: Positions, b: Positions) => a.email!.length - b.email!.length,
		width: -1,
	},
	{
		title: "사번",
		...
		resizable: true,
		sorter: (a: Positions, b: Positions) =>
			a.employeeNumber!.length - b.employeeNumber!.length,
		width: -1,
	},
])
...
const setClassName = (record: Positions, index: Key[]) => {
	return index === selectedRowKeys.value ? "active-row" : ""
}
...
const userModalOpen = async () => {
	...
	await getDepartmentEmployees(params.id).finally(() => (showUserModal.value = true))
}
...
const submitUser = (v: AddUserRef) => {
	userValidate()
		.then(() => {
			putPositionAssignEmployees(v).finally(() => userModalClose())
		})
		.catch((err) => {
 ...
 * @param data
 */
const putPositionAssignEmployees = async (data: AddUserRef) => {
	...
	return await useCFetch<Response<any>>(`/api/v2/settings/positions/assign/employees`, {
		method: "PUT",
		body,
...
<template>
	...
	<position-change-modal
		...
		@update:show="(value) => (showPositionModal = value)"
