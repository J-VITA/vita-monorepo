...
const {
	...
	error,
	refresh,
} = await useAsyncData(`modal-job-titles-list`, () =>
	useCFetch<Response<any>>("/api/v2/settings/jobTitles", {
		method: "GET",
		params: {
			companyCode: companyCode,
			used: true,
...
const ok = () => {
	if (selectedRowKeys.value === data.member[0].jobTitleId) {
		message.warning("동일한 직책 입니다.")
	} else {
		if (selectedRowName.value) {
			Modal.confirm({
				...
				content: createVNode(
					...
					`선택된 사용자의 직책을 '${selectedRowName.value}'(으)로 변경할까요?`
				),
				onOk() {
					loading.value = true
					...
					useCFetch<Response<any>>(`/api/v2/settings/jobTitles/reassign/employees`, {
						method: "PUT",
						body,
					})
...
<template>
	...
	>
		<template #title>
			<a-typography-title :level="3" class="mb-none"> 직책 변경 </a-typography-title>
		</template>
		<a-row :gutter="[15, 15]">
			<a-col flex="auto">
				<a-table
			...
			<a-col flex="1">
				<a-table
					size="small"
					:data-source="rowData"
					:columns="[{ title: '직책', dataIndex: 'name' }]"
					row-key="id"
					:pagination="false"
					:custom-row="
						(record, idx) => {
