...
const { checkPaymentDate } = useExpenseRules()
	...
	if (slips.id) {
		...
		await useCFetch<Response<any>>(`/api/v2/slips/expenses/${slips.id}`, {
			...
			.finally(async () => {
				...
				await checkPaymentDate(SlipType.CARD).then((res) => {
					if (res) {
						if (
							!formData.value.slipHeader.paymentDueDate ||
							!dayjs(formData.value.slipHeader.paymentDueDate).isValid()
						) {
							formData.value.slipHeader.paymentDueDate = dayjs(
			...
			<a-col :span="formData.slipHeader.divisionSlipFlag ? 16 : 13">
				...
				>
					...
					<a-descriptions-item label="지급예정일">
						<a-date-picker
							class="full-width"
							v-model:value="formData.slipHeader.paymentDueDate"
							disabled
						/>
					</a-descriptions-item>
