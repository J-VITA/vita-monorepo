...
import { ApprovalLineDataType, type FormData } from "@/types/approvals/document"
...
onMounted(async () => {
	formData.value.approvalFormType = SlipFormType.PERSONAL_EXPENSE_FORM

	if (id.value) {
		detail(id.value).then(async (data: any) => await responseResult(data))
	} else {
		getRouteParams(`/approvals/write-${compCase(SlipFormType.PERSONAL_EXPENSE_FORM)}`)
			.then((list: any[]) => {
	...
	<a-spin :spinning="isLoading" tip="데이터를 불러오는 중입니다.">
		<a-row :gutter="[20, 15]">
			<a-col :xs="24" :xxl="18">
              ...
              <approval-line
                type="write"
                :state="formData.documentStatusName"
                v-model:approval-line="formData.approvalDetailRequests"
                v-model:approval-line-refer="formData.approvalLineRefer"
                v-model:approval-line-agree-type="formData.agreementOptionType"
                :line-refers="formData.approvalLineReferrerDtos"
                :next-stage="formData.nextApprovalStage"
                @get-data="setApprovalLine"
              />
              -->
