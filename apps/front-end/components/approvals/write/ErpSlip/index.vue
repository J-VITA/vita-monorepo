...
import ErpSlipDetails from "@/components/approvals/document/ErpSlipDetails.vue"
...
const emit = defineEmits<{
	...
	(e: "erpSlipModalOpen", value: boolean): boolean
	(e: "onDraft"): void
}>()
...
const responseResult = async (form: any) => {
	if (form && form.draftEmployeeId === getEmployeeId.value) {
		formData.value = {
			...form,
			approvalFormType: SlipFormType.ERP_SLIP,
...
onMounted(async () => {
	formData.value.approvalFormType = SlipFormType.ERP_SLIP

	if (id.value) {
		detail(id.value).then(async (data: any) => await responseResult(data))
	} else {
		getRouteParams(`/approvals/write-${compCase(SlipFormType.ERP_SLIP)}`)
			.then((list: any[]) => {
					...
					>
						...
						<a-form-item>
							...
							<erp-slip-details
								...
								@on-history="(opend: boolean) => emit('erpSlipModalOpen', opend)"
								@reload-data="
									() => {
										if (id) {
