...
import BusinessTripList from "@/components/approvals/document/BusinessTripList.vue"
import BusinessTripInfoList from "@/components/approvals/document/BusinessTripInfoList.vue"
...
onMounted(async () => {
	...
	if (id.value) {
		...
		getRouteParams(`/approvals/write-${compCase(SlipFormType.CARD_FORM)}`)
			.then((list: any[]) => {
					...
					>
						...
						<a-form-item>
							...
							<BusinessTripList
								type="write"
								v-model:row-data="formData.expenseList"
								@on-history="(opened: boolean) => emit('historyModalOpen', opened)"
								@reload-data="
									() => {
										if (id) {
											detail(id).then(async (data: any) => await responseResult(data))
