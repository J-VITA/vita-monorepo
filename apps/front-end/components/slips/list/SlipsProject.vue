...
const columnDefs = ref<GridOptions<DataType>["columnDefs"]>([
	...
	{ field: "project", headerName: "프로젝트", rowGroup: true, hide: true },
	{
		field: "slipNo",
		headerName: "전표번호",
		cellRenderer: Link,
...
const onGridReady = async (params: any) => {
	...
	const datasource = {
		getRows: async (params: IServerSideGetRowsParams) => {
			...
			let totalData = { totalAmount: 0, supplyValue: 0, vat: 0 }

			getData.forEach((el) => {
				if (el.totalAmount) {
					totalData.totalAmount += el.totalAmount
				}
				if (el.supplyValue) {
					totalData.supplyValue += el.supplyValue
				}
				if (el.vat) {
					totalData.vat += el.vat
				}
			})
...
onMounted(() => {
	...
	projectData.value = [
		{ label: "해외사업 프로젝트", value: "1" },
		{ label: "신규사업 프로젝트", value: "2" },
		{ label: "A 프로젝트", value: "a" },
		{ label: "B 프로젝트", value: "b" },
		{ label: "C 프로젝트", value: "c" },
	]
	accountData.value = [
