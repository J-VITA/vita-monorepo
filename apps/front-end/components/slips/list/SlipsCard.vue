...
const columnDefs = ref<GridOptions<DataType>["columnDefs"]>([
	{
		...
		cellRendererParams: {
			...
			totalValueGetter: (params: GroupCellRendererParams) => {
				...
				const cardAlias = firstChild ? firstChild.data.cardName : params.value
				return `${cardAlias} 소계`
				// return `${params.value} 소계`
			},
		},
	},
	{ field: "cardNumber", headerName: "카드번호", rowGroup: true, hide: true },
	{ field: "cardName", headerName: "카드별칭" },
	{
		field: "slipNumber",
		headerName: "전표번호",
	...
	{ field: "cardApprovalNumber", headerName: "카드승인번호" },
	{ field: "costCenterName", headerName: "부서" },
...
const onGridReady = async (params: GridReadyEvent<DataType>) => {
	...
	const savedColumnState = await Promise.race([getColumns("slipsCardColumnState")])
	if (savedColumnState) {
...
const onColumnVisible = (event: any) => {
	...
	setColumns("slipsCardColumnState", columnState)
}
...
<template>
	...
	>
		...
		<a-descriptions-item label="카드번호" :span="2">
			<a-input v-model:value="searchParams.cardNumber" />
		</a-descriptions-item>
		<a-descriptions-item
			label="전표유형"
			:span="2"
			...
			:autoGroupColumnDef="{
				...
				cellRendererParams: {
					innerRenderer: (params: any) => {
						return params.node.group ? params.data.cardName : params.value
					},
				},
			}"
