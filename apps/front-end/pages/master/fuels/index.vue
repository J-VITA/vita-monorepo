...
const {
	...
	useIFetch<Response<Array<FuelInfoData>>>("/api/v2/master/oils", {
		method: "GET",
...
const showModal = async (id?: number) => {
	if (id) {
		await useIFetch<Response<FuelInfoData>>(`/api/v2/master/oils/${id}`, {
			method: "GET",
...
const onDelete = (users: number[]) => {
	...
	users.forEach(async (id) => {
		await useIFetch<Response<any>>(`/api/v2/master/oils/${id}`, {
			method: "DELETE",
			body: {
				id: id,
			},
...
const onSubmit = (data: FuelEventParams) => {
	fuelFormRef.value
		...
		.then(async () => {
			...
			delete send.method
			const url = data.id ? `/api/v2/master/oils/${data.id}` : "/api/v2/master/oils"
	...
	<page-layout>
		<template #header>
			<a-space :size="5">
				...
				유류단가를 등록/관리할 수 있습니다. 자차이용시 거리에 따라 리터당 금액을 자동산출
				할 수 있습니다.
			</a-space>
		</template>
		<template #content>
